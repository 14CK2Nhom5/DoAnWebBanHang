   ...
   <ul class="products">
   	<?php
   		...
   		if ($key != '')
   		{
	        ...
	        $result = mysqli_query($connect, "SELECT COUNT(*) as total from sanpham sp where sp.tensp like '%$key%' or sp.masp like '%$key%' or sp.giatien like '%$key%'");
	        ...
	        // Có limit và start rồi thì truy vấn CSDL lấy danh sách tin tức
	   		...
	   		$result = mysqli_query($connect, "SELECT * from sanpham sp, thuonghieu th where (sp.tensp like '%$key%' or sp.masp like '%$key%' or sp.giatien like '%$key%')and sp.thuonghieu = th.mathuonghieu LIMIT $start, $limit");
...
<?php
	...
	if ($current_page > 1 && $total_page > 1){
	    echo '<a href="Search?key='.$key.'&page='.($current_page-1).'">Trang trước</a> | ';
	}
	//cong day
	$i = 1;
	if ($current_page > 1 ) {
	...
	for ($i; $i <= $total_page; $i++){
	    ...
	    if ($i == $current_page){
	        ...
	        echo '<a href="Search?key='.$key.'&page='.$i.'">'.$i.'</a> | ';
	    }
	}
	// nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
	if ($current_page < $total_page && $total_page > 1){
	    echo '<a href="Search?key='.$key.'&page='.($current_page+1).'">Trang sau</a> ';
	}
?>
</div>
